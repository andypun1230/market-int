...
function run() {
  ...
  assert(normalizeWatchlistId('sector', 'Technology') === 'technology', 'sector ids normalize to stable slug');
  assert(normalizeWatchlistId('theme', 'AI Infrastructure') === 'ai-infrastructure', 'theme ids normalize to stable slug');
  assert(buildWatchlistKey('sector', 'Technology') === 'sector:technology', 'sector key is normalized');
  ...
  assert(keys.includes('sector:technology'), 'migrates sector item');
  ...
  assert(keys.filter((key) => key === 'sector:technology').length === 1, 'deduplicates duplicate sectors');
  ...
  assert(buildWatchlistKey(withSector[0].type, withSector[0].id) === 'sector:technology', 'stores normalized sector id');
