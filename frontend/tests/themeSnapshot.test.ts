...
async function run() {
  const snapshot = adaptThemeSnapshot({
    ...
    items: [{ theme_id: 'memory-storage', display_name: 'Memory & Storage', rank: 1, composite_score: 77.1, coverage_ratio: 1,
      definition: { aliases: ['memory-and-storage', 'data-storage'], parent_sector_ids: ['information_technology'], status: 'active' }, performance: { '1d': 1, '1w': 2, '1m': 3, '3m': 4, '6m': 5, '1y': 6 },
  ...
  assert(snapshot?.items[0].aliases.includes('data-storage') && snapshot.items[0].parentSectorIds[0] === 'information_technology' && snapshot.items[0].taxonomyStatus === 'active', 'theme adapter preserves canonical alias and parent metadata for presentation filtering');
