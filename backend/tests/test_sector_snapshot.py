...
class SectorSnapshotTests(unittest.TestCase):
    ...
    def test_snapshot_is_deterministic_complete_and_immutable(self) -> None:
        ...
        self.assertEqual(first.schema_version, 5); self.assertEqual([row["rank"] for row in first.sectors], sorted(row["rank"] for row in first.sectors))
        ...
        self.assertTrue(all(set(row["legacy_rotation_series"]) == {"1W", "1M", "3M"} for row in first.sectors))
        ...
        self.assertTrue(all(item["model_version"] == "sector-relative-trend-momentum-v1" for row in first.sectors for item in row["rotation_series"].values()))
    ...
    def test_routes_share_snapshot_and_warm_reads_make_no_provider_calls(self) -> None:
        ...
        self.assertTrue(all(item["rotation"]["1m"]["formula_version"] == "relative-return-momentum-v1" for item in dashboard.json()["sectors"]))
        self.assertTrue(all(item["canonical_rotation"]["1m"]["model_version"] == "sector-relative-trend-momentum-v1" for item in dashboard.json()["sectors"]))
        ...
        self.assertEqual(rotation.json()["current_point_count"], 11)
        self.assertEqual(rotation.json()["trail_point_count"], 110)
        self.assertEqual(rotation.json()["transition_snapshot_count"], 1)
        self.assertTrue(all(len(points) == 1 for points in rotation.json()["published_snapshot_trails"].values()))
        self.assertEqual(rotation.json()["formula_version"], "sector-relative-trend-momentum-v1")
        self.assertEqual(rotation.json()["rotation_model_version"], "sector-relative-trend-momentum-v1")
        self.assertEqual(rotation.json()["normalization_version"], "zero-centered-rolling-robust-scale-v1")
        self.assertEqual(rotation.json()["profile"], "medium")
        self.assertEqual(rotation.json()["timeframe"], "1M")
        self.assertEqual(rotation.json()["eligible_count"], 11)
        ...
        self.assertEqual({item["interval"] for item in series}, {"1M"})
        self.assertTrue(all(len(item["trail_points"]) == 10 for item in series))
        for item in series:
            ...
            self.assertTrue(all(point["plotted_x"] == point["relative_trend"] and point["plotted_y"] == point["relative_momentum"] for point in points))
            self.assertTrue(all(point["model_version"] == "sector-relative-trend-momentum-v1" for point in points))
    ...
    def test_rotation_uses_only_real_distinct_snapshot_dates(self) -> None:
        ...
        self.assertTrue(all(len(points) == 1 for points in rotation["published_snapshot_trails"].values()))
