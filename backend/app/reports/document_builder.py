...
class DocumentBuilder:
    ...
    def _build_leadership_figures(self, market_date: str) -> None:
        ...
        try:
            ...
            if series_rows:
                source_id = self._source("sector-rotation", "Internal rotation engine", "Adjusted sector ETF Relative Trend and Relative Momentum", snapshot.market_date, "derived_snapshot_series")
                series = [FigureSeries(series_id=str(item.get("entity_id")), label=str(item.get("display_name")), unit="rotation", points=[{"date": point.get("market_date"), "x": point.get("relative_trend"), "y": point.get("relative_momentum"), "quadrant": point.get("quadrant")} for point in item.get("trail_points") or []], source_id=source_id, transformation=str(item.get("normalization_version") or "causal robust normalization around 100")) for item in series_rows]
                self.figures.append(FigureSpec(figure_id="sector-rotation", title="Sector Rotation with Tails", subtitle="Relative Trend and Relative Momentum versus SPY", question_answered="Which sectors have strengthening or weakening benchmark-relative trends?", chart_type="rotation", timeframe="Medium profile", data_series=series, source_ids=[source_id], as_of=snapshot.market_date, observation="Rotation tails show the direction and persistence of each sector ETF's benchmark-relative trend.", interpretation="Relative Momentum describes whether Relative Trend is improving or deteriorating; quadrant position alone is insufficient.", confirmation_condition="A sustained improving-to-leading path with qualifying sector breadth.", risk_condition="A leading-to-weakening path accompanied by deteriorating participation.", quality=self._data_quality(source_id, sum(len(item.points) for item in series), len(series) * 2, "canonical rotation engine")))
        ...
        for item in rotation_items:
            points = [{"date": point.get("market_date"), "x": point.get("relative_trend", point.get("relative_strength")), "y": point.get("relative_momentum"), "quadrant": point.get("quadrant")} for point in item.get("trail_points") or [] if number(point.get("relative_trend", point.get("relative_strength"))) is not None and number(point.get("relative_momentum")) is not None]
            if points:
