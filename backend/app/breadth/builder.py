...
class BreadthSnapshotBuilder:
    ...
    def build_and_publish(self, universe_name: str = "sp100") -> BreadthSnapshot | None:
        ...
        now = datetime.now(timezone.utc).isoformat()
        snapshot_id = f"breadth-{universe.universe_id}-{market_date}-{hashlib.sha256(result.input_hash.encode()).hexdigest()[:10]}"
        timestamps = [bar.timestamp for bars in histories.values() for bar in bars]
        snapshot = BreadthSnapshot(snapshot_id=snapshot_id, universe_id=universe.universe_id, universe_version=universe.version, market_date=market_date, created_at=now, published_at=now, status=result.coverage["coverage_status"], score=result.score, classification=result.classification, trend=result.trend, confidence=result.confidence, coverage=result.coverage, advance_decline={key: result.core[key] for key in ("advancing_count", "declining_count", "unchanged_count", "net_advances", "advance_decline_ratio", "percent_advancing", "percent_declining")}, moving_average_breadth={key: result.core[key] for key in ("percent_above_20ema", "percent_above_50ema", "percent_above_200ema")}, highs_lows={key: result.core[key] for key in ("new_52_week_highs", "new_52_week_lows", "highs_minus_lows", "high_low_ratio")}, sector_breadth=result.sectors, divergences=detect_divergence(benchmark, self.storage.history(universe.universe_id, "breadth_score", 30)), source_state=source_state(), providers=["polygon"], latest_input_timestamp=max(timestamps) if timestamps else None, oldest_input_timestamp=min(timestamps) if timestamps else None, timestamp_skew=None, warnings=result.warnings, missing_dependencies=result.coverage["members_missing"], calculation_version=BreadthPolicy.from_environment().calculation_version, input_hash=result.input_hash, sections={"core": section(result.coverage, result.core), "sectors": section(result.coverage, result.sectors), "divergence": section(result.coverage, [])})
